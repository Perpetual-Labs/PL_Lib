...
model MassFlowToPressureAdapter
  ...
  PL_Lib.Components.SensD sensD(redeclare package Medium = Medium) annotation (
...
equation
  ...
  connect(sensT.outlet, sinkMassFlow.flange) annotation (
    Line(points = {{-24, 0}, {60, 0}}, color = {159, 159, 223}));
  connect(m_dot, sinkMassFlow.in_w0) annotation (
    Line(points = {{110, 20}, {64, 20}, {64, 5}}, color = {0, 0, 127}));
  connect(sensT.outlet, sensP.flange) annotation (
    Line(points = {{-24, 0}, {10, 0}, {10, 60}}, color = {159, 159, 223}));
  connect(sensP.p, p) annotation (
    Line(points = {{17, 70}, {110, 70}}, color = {0, 0, 127}));
  connect(sensT.T, T) annotation (
    Line(points = {{-22, 10}, {-10, 10}, {-10, 90}, {110, 90}}, color = {0, 0, 127}));
  connect(sensD.d, d) annotation (
    Line(points = {{37, 50}, {109, 50}}, color = {0, 0, 127}));
  connect(sensT.outlet, sensD.flange) annotation (
    Line(points = {{-24, 0}, {30, 0}, {30, 40}}, color = {159, 159, 223}));
  annotation (

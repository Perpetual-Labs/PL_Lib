...
model MassFlowToPressureAdaptor
  ...
  Modelica.Blocks.Interfaces.RealOutput p(final quantity = "Pressure", final unit = "Pa", displayUnit = "bar", min = 0) "Pressure in port medium" annotation(
    Placement(visible = true, transformation(origin = {110, 50}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {30, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealOutput T(final quantity = "Temperature", final unit = "K", displayUnit = "degC", min = 0) "Temperature in port medium" annotation(
    Placement(visible = true, transformation(origin = {110, 70}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {30, 70}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
...
equation
  connect(flange, sensT.inlet) annotation(
    Line(points = {{-60, 0}, {-36, 0}}, color = {159, 159, 223}));
  connect(sensT.outlet, sinkMassFlow.flange) annotation(
    Line(points = {{-24, 0}, {30, 0}}, color = {159, 159, 223}));
  connect(m_dot, sinkMassFlow.in_w0) annotation(
    Line(points = {{110, 20}, {34, 20}, {34, 6}}, color = {0, 0, 127}));
  connect(sensT.outlet, sensP.flange) annotation(
    Line(points = {{-24, 0}, {10, 0}, {10, 40}}, color = {159, 159, 223}));
  connect(sensP.p, p) annotation(
    Line(points = {{17, 50}, {110, 50}}, color = {0, 0, 127}));
  connect(sensT.T, T) annotation(
    Line(points = {{-22, 10}, {-10, 10}, {-10, 70}, {110, 70}}, color = {0, 0, 127}));
  annotation(
    Icon(graphics = {Rectangle(lineColor = {159, 159, 223}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, lineThickness = 0.5, extent = {{-20, 100}, {20, -100}}, radius = 10), Text(origin = {0, 70}, extent = {{-20, 10}, {20, -10}}, textString = "T"), Text(origin = {0, -70}, extent = {{-20, 10}, {20, -10}}, textString = "m_dot"), Text(origin = {0, 40}, extent = {{-20, 10}, {20, -10}}, textString = "p")}));

...
model TestGasFlow1DFV_A_HXblock2 "Test case for Gas.Flow1DFV"
  ...
  ThermoPower.Gas.SinkPressure sinkP_gas2out(redeclare package Medium = Medium, p0 = 10000, T = 300) annotation(
    Placement(visible = true, transformation(extent = {{120, -70}, {140, -50}}, rotation = 0)));
  ...
  ThermoPower.Gas.SensT sensT_gas2out(redeclare package Medium = Medium) annotation(
    Placement(visible = true, transformation(extent = {{90, -66}, {110, -46}}, rotation = 0)));
  ThermoPower.Gas.ValveLin valve(redeclare package Medium = Medium, Kv = whex / phex) annotation(
    Placement(visible = true, transformation(extent = {{60, -70}, {80, -50}}, rotation = 0)));
  ...
  Modelica.Blocks.Sources.Step Step2(height = -0.2, offset = 1, startTime = 40) annotation(
    Placement(visible = true, transformation(extent = {{40, -50}, {60, -30}}, rotation = 0)));
  ...
  PL_Lib.Components.HeatExchanger PHX(redeclare package GasMedium1 = Medium, redeclare package GasMedium2 = Medium, Dext = 0.012, Dint = 0.01, Lb = 2, Lt = 3, Nr = 10, Nt = 250, Sb = 8, StaticGasBalances = false, cm = 650, rhom(displayUnit = "kg/m3") = 7800) annotation(
...
equation
  ...
  connect(valve.outlet, sensT_gas2out.inlet) annotation(
    Line(points = {{80, -60}, {94, -60}}, color = {159, 159, 223}, thickness = 0.5));
  connect(sensT_gas2out.outlet, sinkP_gas2out.flange) annotation(
    Line(points = {{106, -60}, {120, -60}}, color = {159, 159, 223}, thickness = 0.5));
  ...
  connect(Step2.y, valve.cmd) annotation(
    Line(points = {{61, -40}, {70, -40}, {70, -53}}, color = {0, 0, 127}));
//  Mhex = hex.M;
...
//  Merr = Mhex - Mbal;
  ...
  connect(stateReader_gas1.outlet, valve.inlet) annotation(
    Line(points = {{42, -60}, {60, -60}}, color = {159, 159, 223}));
...
//initial equation
//  Mbal = Mhex;
  connect(stateReader_gas2.outlet, PHX.gas2In) annotation(
    Line(points = {{-34, 40}, {0, 40}, {0, 20}}, color = {159, 159, 223}));
  connect(PHX.gas2Out, stateReader_gas1.inlet) annotation(
    Line(points = {{0, -20}, {0, -60}, {30, -60}}, color = {159, 159, 223}));
  connect(stateReader_gas.outlet, PHX.gasIn) annotation(
    Line(points = {{-34, 0}, {-20, 0}}, color = {159, 159, 223}));
  connect(PHX.gasOut, stateReader_gas3.inlet) annotation(
...
end TestGasFlow1DFV_A_HXblock2;

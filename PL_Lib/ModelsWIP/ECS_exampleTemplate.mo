...
model ECS_exampleTemplate
  extends Modelica.Icons.Example;
  inner ThermoPower.System system annotation (
    Placement(visible = true, transformation(extent={{180,140},{200,160}},        rotation = 0)));
  ...
  Modelica.Blocks.Sources.Ramp ramp_T_RAin(duration = 600, height = -45, offset = Thex_in_RA, startTime = 300) annotation (
    Placement(visible = true, transformation(origin={-244,100},    extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Sources.Ramp ramp_P_RAin(duration = 600, height = -74825, offset = 101325, startTime = 300) annotation (
    Placement(visible = true, transformation(origin={-244,70},    extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  replaceable Components.HX_1DCoFlow PHX(redeclare package ColdFluid = Medium, redeclare package HotFluid = Medium) constrainedby Interfaces.HeatExchangerBase annotation (Placement(transformation(extent={{-90,-16},{-50,24}})));
protected
...
equation
  connect(ramp_T_RAin.y, sourceP_RAin.in_T) annotation (
    Line(points={{-233,100},{-220,100},{-220,59}},        color = {0, 0, 127}));
  connect(ramp_P_RAin.y, sourceP_RAin.in_p0) annotation (
    Line(points={{-233,70},{-226,70},{-226,56.4}},      color = {0, 0, 127}));
  connect(ramp_P_RAin.y, sinkP_RAout1.in_p0) annotation (
    Line(points={{-233,70},{3.55,70},{3.55,45.95}},
                                                  color = {0, 0, 127}));
  connect(throughMassFlow_BAin.outlet, stateReader_BAin1.inlet) annotation (
    Line(points={{-150,-40},{-136,-40}},     color = {159, 159, 223}));
  ...
  connect(throughMassFlow_RAin.outlet, flowSplit.inlet) annotation (
    Line(points={{-180,50},{-166,50}},      color = {159, 159, 223}));
  ...
  connect(flowSplit.outlet1, throughMassFlow.inlet) annotation (
    Line(points={{-154,54},{-150,54},{-150,60},{-140,60}},         color = {159, 159, 223}));
  connect(flowSplit.outlet2, stateReader_RAin1.inlet) annotation (
    Line(points={{-154,46},{-150,46},{-150,40},{-136,40}},         color = {159, 159, 223}));
  connect(ramp_P_RAin.y, sinkP_RAout2.in_p0) annotation (
    Line(points={{-233,70},{123.55,70},{123.55,45.95}},
                                                      color = {0, 0, 127}));
  ...
  connect(sourceP_RAin.flange, throughMassFlow_RAin.inlet) annotation (
    Line(points={{-210,50},{-200,50}},      color = {159, 159, 223}));
  connect(sourceP_BAin.flange, throughMassFlow_BAin.inlet) annotation (
    Line(points={{-180,-40},{-170,-40}},      color = {159, 159, 223}));
  ...
  connect(throughMassFlow.outlet, stateReader_RAin2.inlet) annotation (
    Line(points={{-120,60},{26,60},{26,40},{34,40}},         color = {159, 159, 223}));
  ...
  annotation (
    Diagram(coordinateSystem(extent={{-200,-140},{200,160}}),      graphics={  Text(origin={-180,-20},    lineColor = {170, 0, 0}, extent = {{-30, 10}, {30, -10}}, textString = "Bleed air (hot side)", horizontalAlignment = TextAlignment.Left), Text(origin={-184,80},    lineColor = {0, 85, 255}, extent = {{-30, 10}, {30, -10}}, textString = "Ram air (cold side)",

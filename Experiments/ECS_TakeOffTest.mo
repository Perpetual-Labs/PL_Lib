...
model ECS_TakeOffTest
  ...
  parameter Modelica.SIunits.MassFlowRate whex_RA=0.025 "nominal (and initial) mass flow rate";
  parameter Modelica.SIunits.MassFlowRate whex_BA=0.025 "nominal (and initial) mass flow rate";
  ...
  replaceable Configurations.ECS_hybridConfig_dummy ECS_config(
    redeclare package ColdFluid = ColdFluid,
    redeclare package HotFluid = HotFluid,
    whex_cold=whex_RA,
    whex_hot=whex_BA) constrainedby Templates.ECS_lossless(redeclare package HotFluid = HotFluid, redeclare package ColdFluid = ColdFluid) annotation (Placement(transformation(extent={{-30,-30},{30,30}})));
  ...
  replaceable Modelica.Blocks.Sources.Constant inputT_hot_in(k=Thex_in_BA) constrainedby Modelica.Blocks.Interfaces.SO annotation (Placement(transformation(extent={{-90,-30},{-70,-10}})));
  replaceable Modelica.Blocks.Sources.Constant inputP_hot_in(k=phex_BA) constrainedby Modelica.Blocks.Interfaces.SO annotation (Placement(transformation(extent={{-90,-70},{-70,-50}})));
...
equation
  ...
  connect(ECS_config.signalBus, DataLogger) annotation (Line(points={{30,0},{60,0}}, color={255,204,51}));
